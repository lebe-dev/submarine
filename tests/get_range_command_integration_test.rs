...
fn run_get_range_command(file: &str, range: &str) -> std::process::Output {
    Command::new("cargo")
        .args(&["run", "--bin", "sm", "--", "get-range", file, range])
        .output()
...
#[test]
fn test_get_range_all_exist() {
    let file = test_data_path("valid/simple.srt");

    // Get all 3 subtitles (1-3)
    let output = run_get_range_command(&file, "1-3");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);

    // Check all three subtitles are present
...
fn test_get_range_partial_overlap() {
    ...
    let output = run_get_range_command(&file, "2-4");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_get_range_no_subtitles() {
    ...
    let output = run_get_range_command(&file, "3-4");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
#[test]
fn test_get_range_single_subtitle() {
    let file = test_data_path("valid/simple.srt");

    // Request range 2-2 (single subtitle)
    let output = run_get_range_command(&file, "2-2");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);

    // Should contain only subtitle 2
    assert!(stdout.contains("2\n"));
    assert!(stdout.contains("Second subtitle"));

    // Should NOT contain other subtitles
    assert!(!stdout.contains("First subtitle"));
    assert!(!stdout.contains("Third subtitle"));
}
...
fn test_get_range_subset() {
    ...
    let output = run_get_range_command(&file, "1-2");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_get_range_multiline_text() {
    ...
    let output = run_get_range_command(&file, "1-1");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_get_range_beyond_end() {
    ...
    let output = run_get_range_command(&file, "2-100");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
#[test]
fn test_get_range_invalid_order() {
    let file = test_data_path("valid/simple.srt");

    // Start > end (invalid range)
    let output = run_get_range_command(&file, "5-2");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);

    assert!(stderr.contains("error"));
    assert!(stderr.contains("Start index must be <= end index"));
    assert!(stderr.contains("5") && stderr.contains("2"));
}
...
fn test_get_range_start_zero() {
    ...
    let output = run_get_range_command(&file, "0-5");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn test_get_range_end_zero() {
    ...
    let output = run_get_range_command(&file, "1-0");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
...
#[test]
fn test_get_range_file_not_found() {
    let output = run_get_range_command("nonexistent.srt", "1-5");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
...
#[test]
fn test_get_range_path_traversal() {
    let output = run_get_range_command("../../../etc/passwd", "1-5");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
...
#[test]
fn test_get_range_malformed_file() {
    let file = test_data_path("invalid/malformed.srt");

    let output = run_get_range_command(&file, "1-5");
    assert!(!output.status.success());
...
#[test]
fn test_get_range_with_absolute_path() {
    use std::fs;
    ...
    let output = run_get_range_command(temp_file.to_str().unwrap(), "1-2");
    assert!(output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
...
#[test]
fn test_get_range_invalid_format() {
    let file = test_data_path("valid/simple.srt");

    // Test various invalid formats
    let output = run_get_range_command(&file, "123");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    let output = run_get_range_command(&file, "abc-123");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Invalid"));

    let output = run_get_range_command(&file, "123-abc");
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Invalid"));
}
