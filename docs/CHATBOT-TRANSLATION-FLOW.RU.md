...
Сначала необходимо экспортировать субтитры в специальном формате (anchored). Этот формат подходит для любых популярных LLM, так как исходный формат [SubRip](https://en.wikipedia.org/wiki/SubRip) слишком сложен для современных языковых моделей (ломаются timestamps, пропускаются индексы).
...
```prompt
Давай переведем субтитры с английского на русский для сериала Resident Alien, 3 сезон, 8 серия.
...
Либо отредактируйте файл вручную. Для последующих фрагментов достаточно отправлять чатботу только сами фрагменты без лишних пояснений. Дальше скормите чатботу следующие фрагменты `101-200`, `201-300` и так до конца. Всегда добавляйте содержимое в файл `chatbot-import.md`.
