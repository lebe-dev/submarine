    ...
    match log_target {
        "file" => {
            ...
            Config::builder()
                ...
                .unwrap_or_else(|_| panic!("unable to create log file '{}'", log_path.display()))
        }
        _ => Config::builder()
...
fn get_rolling_appender(level: LevelFilter) -> Appender {
    ...
    let log_file_path_str = log_file_path
        .to_str()
        .expect("unable to convert log path to string");
    let log_file_format = format!("{}.{{}}", log_file_path_str);
    ...
    let rolling_appender = RollingFileAppender::builder()
        .encoder(get_encoder())
        .build(log_file_path_str, Box::new(policy))
